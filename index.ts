...
app.post("/todos", (req: any, res: any) => {
  ...
  const iD: string = random.toString() + i.toString();
  req.body.id = iD;
  i++;
  toDos.push(req.body);
...
app.get("/todos/:id", (req: any, res: any) => {
  let tid: string = req.params.id;

  // let foundTodo:any = {};
  let i = 0;
  for (; i < toDos.length; i++) {
    let check: string = toDos[i].id;
...
app.put("/todos/:tid", (req: any, res: any) => {
...
app.delete("/todos/:tid", (req: any, res: any) => {
  const tid: string = req.params.tid;
  const arr = [];
  let count = 0;
  for (let i = 0; i < toDos.length; i++) {
